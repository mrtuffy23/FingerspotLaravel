...
<div class="container-fluid py-4">
    ...
    <h2 class="fw-bold mb-4">
        <i class="bi bi-pencil-square me-2"></i> Edit Izin Lembur
    </h2>

    <!-- FORM CARD -->
    <div class="card shadow-lg border-0 rounded-4">
        <div class="card-header bg-warning text-white py-3 rounded-top-4">
            <h5 class="mb-0">Edit Izin Lembur #{{ $overtimePermit->id }}</h5>
        </div>

        <div class="card-body px-4 py-4">
            <form action="{{ route('overtime-permit.update', $overtimePermit) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                        ...
                        <select name="employee_id" class="form-select form-select-lg @error('employee_id') is-invalid @enderror" required>
                            ...
                            @foreach ($employees as $employee)
                                ...
                                <option value="{{ $employee->id }}" 
                                    {{ old('employee_id', $overtimePermit->employee_id) == $employee->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="date" class="form-control form-control-lg @error('date') is-invalid @enderror" 
                            value="{{ old('date', $overtimePermit->date->format('Y-m-d')) }}" required>
                        @error('date')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="time" name="overtime_end_time" class="form-control form-control-lg @error('overtime_end_time') is-invalid @enderror" 
                            value="{{ old('overtime_end_time', $overtimePermit->overtime_end_time) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="reason" class="form-control form-control-lg @error('reason') is-invalid @enderror" 
                            placeholder="Masukkan alasan lembur" value="{{ old('reason', $overtimePermit->reason) }}">
                        @error('reason')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 mt-4">
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('overtime-permit.show', $overtimePermit) }}" class="btn btn-secondary btn-lg">
